...
默认语言使用根路径，其他语言使用 `/<locale>/` 前缀。Docfuse 不会根据浏览器语言自动跳转，访问 `/` 时始终显示 `defaultLocale`。

## 目录结构

`docfuse init` 把默认语言放在 `docs/` 根目录，只为其他语言创建子目录。以中文为默认语言时，目录可以这样组织：
...
也可以把默认语言放在显式目录中，例如 `docs/zh/index.md`。Docfuse 官网采用这种对称布局。两种结构都受支持，但不能同时创建 `docs/index.md` 和 `docs/zh/index.md`，否则它们都会映射到 `/`，检查和构建会报告路由冲突。
...
运行 `docfuse check` 后，Docfuse 会按相对路径比较各语言的页面并报告缺失项。页面标题、正文、侧边栏文案和截图不会自动翻译，需要在每种语言中分别维护。
