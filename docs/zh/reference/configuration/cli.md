---
title: CLI 命令参考
description: 查看 docfuse init、dev、check、build、preview 和 deploy 的输入、输出与失败条件
group: 参考
subgroup: 配置与命令
order: 53
---
...
本页用裸 `docfuse` 表示命令签名。项目脚本可直接使用这个写法；临时执行本地依赖时在前面加 `pnpm exec`。安装方式见[安装](/guide/introduction/installation/)。

```bash
docfuse --help
```

Docfuse 0.1 提供七个命令。`dev`、`check`、`build`、`clean`、`preview` 和 `deploy` 从当前工作目录加载配置；支持 `docfuse.config.ts`、`.mts`、`.cts`、`.js`、`.mjs` 和 `.cjs`。`init` 在目标目录创建或读取配置，`--help` 不加载项目配置。

| 命令 | 用途 | 默认输出 |
|---|---|---|
| `docfuse init [dir] [options]` | 创建文档或接入已有内容 | 内容目录、`docfuse.config.ts` |
| `docfuse dev [--port]` | 启动本地开发服务 | `http://127.0.0.1:3333/` |
| `docfuse check` | 输出内容质量问题 | 终端报告 |
| `docfuse build [--no-cache]` | 生成静态站点 | `.docfuse/dist` |
| `docfuse clean` | 删除生成产物和构建状态 | — |
| `docfuse preview [--port]` | 预览已构建产物 | 本地 HTTP 服务 |
| `docfuse deploy` | 生成托管平台配置示例 | `.docfuse/deploy` |
...
docfuse init --locale zh --locales zh,en
docfuse init --locale zh --docs-dir handbook
```

- 不传 `dir` 时，在当前目录创建文档文件。
- 传入 `dir` 时，在子目录中创建独立项目。
- 全新项目默认创建中文单语言内容，直接位于 `docs/`，不创建 `zh/` 或 `en/`。
- 已有 Markdown/MDX 时原地接入，只补充缺失的配置与 `docfuse-env.d.ts`，不移动、覆盖或自动补写内容。
- 已有 `docfuse.config.ts` 时以配置为准；重复运行是成功的无操作。
...
启动时先构建一次，然后监听项目输入。监听范围覆盖配置的内容目录、本地样式、组件、Extension，以及六种受支持的配置文件；自动排除 `.git`、`node_modules`、`.docfuse`、构建输出和临时交换目录。
...
- `docfuse.config.ts` 无法加载或不符合配置 schema。
- `requiredVersion` 与当前 Docfuse 版本不匹配。
- Markdown、MDX 或本地 React 组件编译失败。
- 路由、输出文件、顶部导航或分组元数据冲突。
- 版本、重定向或语言配置不成立。

Docfuse 在 `.docfuse/cache` 保存版本化 Manifest。缓存命中前会同时校验输入和生成文件；
