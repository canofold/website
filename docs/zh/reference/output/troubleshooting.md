---
title: 故障排查
description: 诊断 Canofold 配置、路由、Markdown、MDX、图表、搜索和部署问题
group: 参考
subgroup: 产物与质量
order: 73
---
...
- `canofold check` 只校验配置和插件声明。使用 `canofold dev` 或 `canofold build` 验证实际转换。
- 自定义插件的 `name` 必须唯一。转换代码变化时提升 `version`，选项变化必须反映到 `cacheKey`。

如果检查结果与页面不一致，运行 `pnpm exec canofold build --no-cache` 排除旧构建状态。浏览器端能力仍失败时检查控制台和 Network 面板，确认插件脚本及样式没有 404 或被 CSP 拦截。
...
- `canofold check` 不执行 `generate`。检查附加文件时运行 `canofold build`，并到 `outputDir/extensions/{name}/` 查看结果。
...
- Extension 通过 `extendPage` 修改 `searchText` 后，需要重新构建搜索索引；只运行 `canofold check` 不会写入索引。

## 部署后 404

静态主机必须按目录提供 `index.html`。确认发布目录与 `outputDir` 一致，且没有把站点配置成 SPA fallback。运行 `canofold deploy` 查看平台示例。
